...
use nalgebra::{DMatrix, DVector, Dyn, MatrixView};
...
#[derive(Debug)]
pub struct Imvgmm {
    mixture_model: Option<ConjugateMixtureModel<DVector<f64>, MvGaussian, NormalInvWishart>>,
    rng: rand::rngs::SmallRng,
    sampler: PartitionGibbs,
    data: Vec<Vec<f64>>,
}
...
impl Imvgmm {
    #[new]
    #[pyo3(signature = (data, alpha = 1.0, seed = None))]
    fn new(data: Vec<Vec<f64>>, alpha: f64, seed: Option<u64>) -> Self {
        let mut rng = seed.map_or_else(
            || rand::rngs::SmallRng::from_entropy(),
            |s| rand::rngs::SmallRng::seed_from_u64(s),
        );
        Self {
            mixture_model: Some(ConjugateMixtureModel::new(
                ...
                data.iter().map(|x| DVector::from(x)),
                alpha,
                &mut rng,
            )),
            rng,
...
/// * `n_mcmc_iters` - Number of MCMC iterations to take.
/// * `n_mc_delta_iters` - Number of MC iterations to estimate each delta.
...
/// * `responses` - Output of the function
/// * `n_mcmc_iters` - Number of MCMC iterations to take.
/// * `n_mc_delta_iters` - Number of MC iterations to estimate each delta.
/// * `warmup` - Number of MCMC iterations to discard at startup.
/// * `thinning` - Number of MCMC iterations to discard between steps.
/// * `seed` - An optional seed for the rng.
#[pyfunction]
fn single_loop_deltas_from_outputs<'a>(
    py: Python<'a>,
    parameters: &'a Bound<PyArray2<f64>>,
    responses: &'a Bound<PyArray1<f64>>,
    n_mcmc_iters: usize,
    n_mc_delta_iters: usize,
    warmup: usize,
    thinning: NonZeroUsize,
...
fn migsa(_py: Python, m: &Bound<PyModule>) -> PyResult<()> {
    ...
    m.add_function(wrap_pyfunction!(single_loop_deltas_from_outputs, m)?)?;
    Ok(())
}
